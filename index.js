...
// 	newArr.map((number) => {
// 		secondArr.push(number * 2);
// 	});
// 	console.log(secondArr);
// }
// mapFunction();

function copy(flavArr) {
	const flavCopy = [];

	flavArr.map((flavor) => {
		return flavCopy.push(flavor);
	});
	return flavCopy;
}
/*🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀 Task 2: 🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀 
...
function is31Flavors(flavBoo) {
	if (flavBoo.length === 31) {
		return true;
	}
}
...
function addFlavor(addArray, flavStr) {
	addArray.unshift(flavStr);
	return addArray;
}
...
function removeLastFlavor(remFlav) {
	remFlav.pop();
	return remFlav;
}
...
function getFlavorByIndex(addFlav, flavStr) {
	addFlav.splice(flavStr, 0);
	// console.log(flavStr);
	// console.log(addFlav);
	return addFlav[flavStr];
}
...
function removeFlavorByName(delFlav, delStr) {
	const delIn = delFlav.indexOf(delStr);
	// console.log(delIn);
	delFlav.indexOf(delStr);
	delFlav.splice(delIn, 1);
	// console.log(delFlav);
	return delFlav;
}
...
function filterByWord(flavArr, flavStr) {
	const retArr = [];

	for (let i = 0; i < flavArr.length; i++) {
		if (flavArr[i].includes(flavStr)) {
			retArr.push(flavArr[i]);
		}
	}
	return retArr;
}
